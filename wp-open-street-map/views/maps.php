...
<a href="<?= wp_nonce_url(admin_url('admin.php?page=wp_openstreetmaps&task=new'), 'edit_wposm') ?>" class="button button-primary"><?php _e('Add a new map', 'wp-open-street-map') ?></a>
<?php
	...
	if(sizeof($maps) > 0)
	{
		foreach($maps as $map)
		{
			...
			<a href="'.wp_nonce_url(admin_url('admin.php?page=wp_openstreetmaps&task=manage&id='.$map->id), 'edit_wposm').'" title="Manage markers"><img src="'.plugins_url( 'images/manage.png', dirname(__FILE__) ).'" /></a>
			<a href="'.wp_nonce_url(admin_url('admin.php?page=wp_openstreetmaps&task=remove&id='.$map->id), 'remove_wposm' ).'" title="Remove map" class="remove_wp_osm"><img src="'.plugins_url( 'images/remove.png', dirname(__FILE__) ).'" /></a>
			<br />
			<b>Shortcode : </b>
